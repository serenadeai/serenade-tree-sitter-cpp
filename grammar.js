...
module.exports = grammar(C, {
  ...
  rules: {
    ...
    decltype: $ => seq('decltype', '(', $.expression_, ')'),
    ...
    _type_specifier: $ =>
      choice(
        ...
        $.dependent_type,
        $.decltype,
        prec.right(choice($.scoped_type_identifier, $._type_identifier))
      ),

    type_qualifier: ($, original) =>
      field('modifier', choice(original, 'mutable', 'constexpr')),
    ...
    _class_name: $ =>
      prec.right(
        choice($._type_identifier, $.scoped_type_identifier, $.template_type)
      ),
    ...
    virtual_function_specifier: $ => field('modifier', choice('virtual')),

    explicit_function_specifier: $ =>
      choice(
        'explicit',
        prec(PREC.CALL, seq('explicit', '(', $.expression_, ')'))
      ),

    base_class_clause: $ =>
      seq(':', field('extends_list', commaSep1($.extends_type))),
    ...
    storage_class_specifier: $ =>
      field('modifier', choice('extern', 'static', 'register', 'inline')),

    auto: $ => 'auto',

    dependent_type: $ => prec.dynamic(-1, seq('typename', $._type_specifier)),

    // Declarations

    function_definition: ($, original) => seq(repeat($.attribute), original),

    declaration_without_semicolon: ($, original) =>
      seq(repeat($.attribute), original),
    ...
    parameter_declaration: ($, original) => seq(repeat($.attribute), original),
    ...
    variadic_type_parameter_declaration: $ =>
      prec(
        1,
        seq(choice('typename', 'class'), '...', optional($._type_identifier))
      ),
    ...
    variadic_reference_declarator: $ =>
      seq(choice('&&', '&'), $.variadic_declarator),
    ...
    init_list_declarator: $ =>
      seq(
        ...
        field(
          'assignment_value',
          choice($.argument_list_block, $.initializer_list)
        )
      ),

    init_declarator: ($, original) => choice(original, $.init_list_declarator),
    ...
    field_declaration_list_item_: ($, original) =>
      choice(
        original,
        $.template_declaration,
    ...
    field_declaration_maybe_assignment: $ =>
      seq(
        field(
          ...
          commaSep1(alias($.field_declarator, $.maybe_assignment_variable))
        ),
        optional_with_placeholder(
          'assignment_value_list_optional',
          $.field_declaration_assignment_value
        )
      ),

    field_declaration_assignment_value_equal_variant: $ =>
      seq(
        '=',
        field('assignment_value', choice($.expression_, $.initializer_list))
      ),
    ...
    _abstract_declarator: ($, original) =>
      choice(original, $.abstract_reference_declarator),

    reference_declarator: $ =>
      prec.dynamic(1, prec.right(seq(choice('&', '&&'), $._declarator))),
    reference_field_declarator: $ =>
      prec.dynamic(1, prec.right(seq(choice('&', '&&'), $.field_declarator))),
    abstract_reference_declarator: $ =>
      prec.right(seq(choice('&', '&&'), optional($._abstract_declarator))),
    ...
    function_field_declarator: ($, original) =>
      prec.dynamic(
        1,
        seq(
          original,
    ...
    abstract_function_declarator: ($, original) =>
      prec.right(
        seq(
          original,
          optional_with_placeholder(
            'function_modifier_list',
            repeat(choice($.type_qualifier, $.noexcept, $.throw_specifier))
          ),
          optional_with_placeholder(
            'return_type_optional',
            $.trailing_return_type
    ...
    noexcept: $ =>
      field(
        'modifier',
        prec.right(
          seq('noexcept', optional(seq('(', optional($.expression_), ')')))
        )
      ),

    throw_specifier: $ =>
      seq('throw', seq('(', commaSep($.type_descriptor), ')')),
    ...
    statement: ($, original) => choice(original, $.try, $.throw),
    ...
    condition_declaration: $ =>
      seq(
        $.declaration_specifiers,
        field('declarator', $._declarator),
        choice(seq('=', $.expression_), $.initializer_list)
      ),

    for: ($, original) => choice(original, $.for_each_clause),
    ...
    for_each_clause: $ =>
      seq(
        ...
        ':',
        field('block_collection', choice($.expression_, $.initializer_list)),
        ')',
        field('body', $.statement)
      ),

    return_value: ($, original) => choice(original, $.initializer_list),
    ...
    try: $ =>
      seq(
        ...
        optional_with_placeholder(
          ...
          '!!UNMATCHABLE_4b0fd36954db'
        ) // Here for Spec purposes.
      ),

    try_clause: $ => seq('try', $.enclosed_body),

    catch_parameter_block: $ =>
      seq('(', alias($.parameter_list, $.catch_parameter), ')'),
    ...
    attribute: $ => seq('[[', commaSep1($.expression_), ']]'),

    // Expressions

    expression_: ($, original) =>
      choice(
        original,
        $.template_function,
        $.scoped_identifier,
    ...
    delete_expression: $ =>
      seq(optional('::'), 'delete', optional(seq('[', ']')), $.expression_),

    field_expression: ($, original) =>
      choice(
        original,
        seq(
          prec(
            PREC.FIELD,
            seq(field('argument_', $.expression_), choice('.', '->'))
          ),
          choice($.destructor_name, $.template_method)
        )
      ),
    ...
    parameter_pack_expansion: $ =>
      prec(-1, seq(field('pattern', $.expression_), '...')),

    type_parameter_pack_expansion: $ =>
      seq(field('pattern', $.type_descriptor), '...'),

    sizeof_expression: ($, original) =>
      choice(original, seq('sizeof', '...', '(', $.identifier, ')')),

    argument: ($, original) => choice(original, $.initializer_list),

    destructor_name: $ => field('identifier', prec(1, seq('~', $.identifier))),

    new_compound_literal_expression: $ =>
      seq(
        field(
          'type',
          choice($._type_identifier, $.template_type, $.scoped_type_identifier)
        ),
        field('value', $.initializer_list)
      ),

    compound_literal_expression: ($, original) =>
      choice(original, $.new_compound_literal_expression),
    ...
    _assignment_left_expression: ($, original) =>
      choice(original, $.scoped_namespace_identifier),
